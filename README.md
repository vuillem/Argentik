...
L’utilisateur affiche un négatif numérique sur l’écran du téléphone puis place le smartphone directement sur le papier photographique. L’écran devient alors la source lumineuse qui expose le papier. L’application permet de contrôler précisément les temps d’exposition et de reproduire les étapes classiques de travail en chambre noire.
...
The user displays a digital negative on the phone’s screen and then places the smartphone directly on the photographic paper. The screen acts as the exposure light source. The application allows precise control of exposure times and reproduces the basic workflow used in a traditional darkroom.
...
### Logique de travail : bandes test avant tirage final / Workflow: test strips before final exposure
...
- time increment between bands  
- number of bands

This allows the user to generate a full exposure scale on a single test print.
...
### Exposition intégrale / Full image exposure
...
2. le négatif apparaît pendant la durée d’exposition programmée  
...
### Présets de contraste / Contrast presets
...
The goal is to help users reach a satisfactory result more quickly during the first test strips.

---

### Repérage des temps d’exposition / Exposure time labels
...
### Manipulation de l’image / Image manipulation tools
...
Ces fonctions permettent d’utiliser le smartphone dans différentes positions sur le papier photographique.
...
In the current version, the smartphone system status bar (time, battery level, notifications) is not automatically hidden. This area may emit unwanted light during exposure.
